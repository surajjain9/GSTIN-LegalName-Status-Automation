...
file = "C:\\YYYY\\XXXX\\OneDrive\\Desktop\\Others\\GST\\GSTIn.xlsx"
...
for r in range(2,row+1):
                ...
                time.sleep(5)
                tradeName = driver.find_element(By.XPATH, "//*[@id='lottable']/div[2]/div/div[2]/p[2]/strong").text
                time.sleep(5)
                PoBClick = driver.find_element(By.XPATH,"//*[@id='lottable']/div[3]/div/ul/li[2]/a").click()
                time.sleep(5)
                PoBextract = driver.find_element(By.XPATH,"//*[@id='auth']/div/div/div/table/tbody/tr/td[3]").text
                writeNB = XLUtility.writeData(file, 'data', r, 2, str(legalName))
                writeStatus = XLUtility.writeData(file, 'data', r, 3, str(status))
                writePoB = XLUtility.writeData(file, 'data', r, 4, str(PoBextract))
                writeTradeName = XLUtility.writeData(file, 'data', r, 5, str(tradeName))
